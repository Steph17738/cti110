...
def main():
    ...
    #numbers allowed, input until one number is chosen
    numbers_allowed = [1.5, 2, 3] 
    while num_people not in numbers_allowed:
        print('INVALID ENTRY!!!')
    ...
    if num_people in numbers_allowed:
        print('----Pizza Order---------')
    ...
    run_again = str(input('Would you like to run the program again? (y for yes): '))
    while True:
        if run_again == 'y':
                main()         
        else:
